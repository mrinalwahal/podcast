# Competitive Landscape

> In a niche this small, "competitors" are also your **guest roster** and **cross-promotion network.** The crowdedness proves the audience exists. Be the sharpest, tightest, most honest node — and plug into the network, don't fight it.

## The role matrix

| Entity | Podcast | Ideology | Role for you |
|---|---|---|---|
| **Seen and the Unseen** (Amit Varma) | Yes (active) | Classical-liberal, literary | Benchmark + guest map + competitor |
| **Everything is Everything** (Varma + Ajay Shah) | **Ended Dec 2025, Ep.128** | Pro-market, frameworks | **Template + orphaned audience to inherit** |
| **Ideas of India** (Shruti Rajagopalan) | Yes | Academic classical-liberal | Direct competitor + top guest |
| **Growth is Good** (Foundation for Economic Development) | Yes | Explicitly pro-market | Closest live competitor + ally + guest |
| **Puliyabaazi** (Chandra, Kotasthane) | Yes (Hindi) | Liberal, policy-nerd | Format proof + ally + guest |
| **All Things Policy** (Takshashila) | Yes (daily) | Independent liberal | Guest bench + partial competitor |
| **Econ Central** (Varma + Vivek Kaul) | Yes | Accessible economics | Register model + guest (Kaul) |
| **Thrive by Groww** / **Let's Talk Money** (Halan) | Yes | Personal finance | Finance benchmark + **Halan = guest** |
| **CCS / Takshashila / Mercatus-EV / XKDR / Artha / Prosperiti** | mixed | Classical-liberal / pro-market | **Allies + deep guest bench** |
| **IVM Podcasts** | (network) | Commercial | Possible distribution partner |
| **ORF PolicyPod / Interpreting India / Grand Tamasha** | Yes | Establishment/centrist | Airtime competitors, **not allies** |

## Direct competitors — study, then differentiate

- **Seen and the Unseen** — the 800-lb gorilla. Weekly, now **3–6 hrs**, single guest, audio-first, ~225k downloads/mo (self-reported). Named after **Bastiat**; Varma is a 2× Bastiat Prize winner. **Learn:** the seen/unseen lens; biography-first openings; **syllabus-grade show notes (copy this).** **Differentiate:** you're a fraction of the length, video-first, warmer, explicitly anti-crony.
- **Everything is Everything** — *your closest template, and it just ended.* Weekly, YouTube-first, ~70 min, writer+economist dialogue, organized around portable **"frames."** **Learn:** that exact chemistry is *your* structure (you = translator, guest = rigor); chaptered episodes. **Inherit:** its orphaned audience.
- **Growth is Good** (FED) — your closest *live* competitor; unabashedly pro-growth, strong guests (Montek, Ajay Shah). **Differentiate:** you're independent, host-driven, broader (liberty + money), more accessible, anti-crony-branded. Also an **ally/guest** target (Rahul Ahluwalia).
- **Ideas of India** (Rajagopalan) — the academic competitor; deeply prepared. **Differentiate:** you sit one notch toward accessible. Rajagopalan is a dream guest + gateway to the EV India talent pool.
- **Puliyabaazi** — the format proof: accessible-but-serious, Hindi, non-preachy — *exactly your tone.* **Differentiate:** English-Hinglish, video-first, tighter POV. Kotasthane = great guest.
- **Econ Central** (Vivek Kaul) — the best template for an accessible *money* register; **Kaul is your ideal inflation/finance guest.**
- **Thrive by Groww / Halan** — the personal-finance benchmark. Not a head-to-head rival (different job). **Monika Halan is a strong guest** for a *finance-as-ideas* episode (insurance mis-selling, why the system is rigged against ordinary buyers).

## Adjacent (same audience, different tilt — benchmarks, not allies)
- **Interpreting India** (Carnegie) — production-quality benchmark.
- **Grand Tamasha** (Milan Vaishnav) — centrist India politics.
- **ORF PolicyPod** — establishment, **Reliance-funded**; be honest, not a pro-market ally.
- **Paisa Vaisa** (IVM) — big personal-finance audience; adjacent.

## Allies & guest pipelines (the network to join)
- **CCS** (Parth Shah) — classical-liberal; deep guest bench + the **IndianLiberals.in** archive.
- **Takshashila** (Nitin Pai, Kotasthane) — public-policy bench + *Anticipating the Unintended* newsletter.
- **Mercatus / Emergent Ventures India** (Rajagopalan) — richest pipeline of *young* pro-market Indians (a story engine).
- **XKDR** (Ajay Shah, Susan Thomas, Anirudh Burman) — no own podcast → **prime available guest pool.**
- **Artha Global** (Niranjan Rajadhyaksha) — media-fluent macro guest.
- **Prosperiti / Trayas** (Bhuvana Anand) — deregulation, labour, land, women's economic freedom.
- **The 1991 Project / IndianLiberals.in** — content, archive, and episode-idea sources.

## Distribution partner (optional, later)
- **IVM Podcasts** — India's pioneering network (a Pratilipi vertical); syndicates *All Things Policy* & *Puliyabaazi*. A credible ad-sales/production partner *if* you ever outsource — **not needed to launch** (self-distribute via one RSS feed).

## How you win in a crowded room
1. Be the **accessible successor to EiE** (tighter, video-first, interview format).
2. Own **"pro-market because anti-crony"** as your front door.
3. **Turn competitors into guests** — their guest lists are your booking roster ([episodes/README.md](../episodes/README.md)).
4. **Cross-promote, don't compete** — clip-swaps with the CCS/Takshashila/Puliyabaazi networks are your cheapest growth.
5. **Out-produce on clarity + honesty** — few shows are both ruthlessly accessible *and* visibly honest about market failures. Be both.
